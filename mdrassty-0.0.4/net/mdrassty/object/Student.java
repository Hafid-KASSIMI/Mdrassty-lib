...
public class Student implements Serializable {
    ...
    private final HashMap<Integer, HashMap<SEMESTER, StudentCCMarks>> ccMarks = new HashMap();
...
//                return Decision.WARNING;
    ...
    public void setCCMark(int matter, SEMESTER semester, int test, float mark) {
        if ( !ccMarks.containsKey(matter) ) {
            ccMarks.put( matter, new HashMap() );
        }
        if ( !ccMarks.get(matter).containsKey(semester) ) {
            ccMarks.get(matter).put( semester, new StudentCCMarks() );
        }
        ccMarks.get(matter).get(semester).setCcMarks(test, mark);
    }
    
    public Float getCCMark(int matter, SEMESTER semester, int test) {
        if ( !ccMarks.containsKey(matter) )
            return null;
        if ( !ccMarks.get(matter).containsKey(semester) )
            return null;
        return ccMarks.get(matter).get(semester).getCcMark(test);
    }
    
    public void setAiMark(int matter, SEMESTER semester, float mark) {
        if ( !ccMarks.containsKey(matter) ) {
            ccMarks.put( matter, new HashMap() );
        }
        if ( !ccMarks.get(matter).containsKey(semester) ) {
        ...
        ccMarks.get(matter).get(semester).setAiMark(mark);
    }
    
    public Float getAiMark(int matter, SEMESTER semester) {
        if ( !ccMarks.containsKey(matter) )
            return null;
        if ( !ccMarks.get(matter).containsKey(semester) )
            return null;
        return ccMarks.get(matter).get(semester).getAiMark();
    }
    
    public ArrayList<Integer> getAssociatedMatters() {
        return ccMarks.keySet().stream().distinct().collect(Collectors.toCollection(ArrayList::new));
    }
    
    public ArrayList<SEMESTER> getAssociatedSemesters() {
        ArrayList<SEMESTER> semesters = new ArrayList();
        if ( ccMarks.values().stream().anyMatch(o -> o.containsKey(SEMESTER.SEMESTER_1)) )
            semesters.add(SEMESTER.SEMESTER_1);
        if ( ccMarks.values().stream().anyMatch(o -> o.containsKey(SEMESTER.SEMESTER_2)) )
            semesters.add(SEMESTER.SEMESTER_2);
        return semesters;
    }
}
